...
void Entropy::Load(Thermal stats) {
    ...
    current = calculateBoltzmannEntropy(stats);
...
{   // S = k*ln*W
    ...
    float hotEntropy = factorial(totalHot) / (factorial(stats.leftHot) * factorial(stats.rightHot));
    float coldEntropy = factorial(totalCold) / (factorial(stats.leftCold) * factorial(stats.rightCold));
